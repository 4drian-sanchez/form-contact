...
global $wpdb;
$tabla = "{$wpdb->prefix}form_contact";
$tabla2 = "{$wpdb->prefix}form_contact_respuestas";


$datos = $wpdb->get_results("select * from {$tabla} order by id desc", ARRAY_A);
...
<main class="container py-3">
    <h1 class="fs-4"><?php echo get_admin_page_title() ?></h1>

    <div class="d-flex justify-content-end">
        <button class=" fs-6 btn btn-primary" type="button" data-bs-toggle="modal" data-bs-target="#form_contact">
    ...
    <!-- MODAL -->
    <div class="modal fade mt-5" id="form_contact" tabindex="-1" aria-labelledby="Formulario del plugin" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">

                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="exampleModalLabel">Crear nuevo formulario </h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form action="" method="POST" name="form_contact" class="p-2" novalidate autocapitalize="off">
                        <div class="mt-3">
                            <label for="nombre" class="form-label">Nombre:</label>
                            <input type="text" placeholder="Escriba su nombre" class="form-control" name="nombre" id="nombre">
                        </div>
                        <div class="mt-3">
                            <label for="correo" class="form-label">Correo:</label>
                            <input type="email" placeholder="Escriba su correo" class="form-control" name="correo" id="correo">
                        </div>
                        <input type="hidden" name="nonce" value="<?php echo wp_create_nonce('seg') ?>">
                        <input class="btn btn-warning text-light mt-3" type="submit" value="Enviar">
                    </form>
                </div>
            </div>
        </div>
    ...
    <table class="table mt-5 table-striped table-hover table-responsive">
        <thead>
        ...
        <tbody>
            ...
            ?>
                <tr>
                    ...
                    <td><?php echo "[form_contact_id={$dato['id']}]" ?></td>
                    <td>
                        <a href="#">
